...
int main(int argc, char *argv[]) {
    ...
    result = rm_change_work_frame(robot_handle, "Base");
    ...
    //Lift up
    result = rm_set_lift_height(robot_handle, 20, 500, true);
   ...
   // Move the robot arm back to the initial position
    result = rm_movel(robot_handle, pose_movej_p, 20, 0, 0, 1);
    if (check_result(result, "Failed to move back to initial position") != 0) {
        return -1;
    }
