...
int main(int argc, char *argv[]) {
    ...
    result = rm_get_current_arm_state(robot_handle, &current_state);
    ...
    SLEEP_S(1);
    rm_pose_t start_pose = { {0.3, 0, 0.4},{0, 0, 0,0}, {3.141, 0, 0} };
    result = rm_movej_p(robot_handle, start_pose, 20, 0, 0, 1);
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        result = rm_set_force_position(robot_handle, 1, 0, 2, -5);
        ...
        result = rm_stop_force_position(robot_handle);
    ...
    printf("Force control demo run successfully...");
